...
class KualiStudent::WorkflowPreferences < KualiStudent::BasePage

  header_elements
  footer_elements

  crucial_element(:frame_el) { |b| b.frame(:class=>"gwt-Frame KS-Action-List") }
  element(:refresh_rate) { |b| b.frame_el.text_field(:name=>"preferences.refreshRate") }
  element(:page_size) { |p| p.frame_el.text_field(:name=>"preferences.pageSize") }
  element(:email_notification) { |b| b.frame_el.select(:name=>"preferences.emailNotification") }
  element(:receive_primary_delegate_emails) { |b| b.frame_el.checkbox(:name=>"preferences.notifyPrimaryDelegation") }
  element(:receive_secondary_delegate_emails) { |b| b.frame_el.checkbox(:name=>"preferences.notifySecondaryDelegation") }
  element(:delegator_filter) { |b| b.frame_el.select(:name=>"preferences.delegatorFilter") }
  element(:primary_delegate_filter) { |b| b.frame_el.select(:name=>"preferences.primaryDelegateFilter") }
  element(:show_document_type) { |b| b.frame_el.checkbox(:name=>"preferences.showDocType") }
  element(:show_title) { |b| b.frame_el.checkbox(:name=>"preferences.showDocTitle") }
  element(:show_actionrequested) { |b| b.frame_el.checkbox(:name=>"preferences.showActionRequested") }
  element(:show_initiator) { |b| b.frame_el.checkbox(:name=>"preferences.showInitiator") }
  element(:show_delegator) { |b| b.frame_el.checkbox(:name=>"preferences.showDelegator") }
  element(:show_date_created) { |b| b.frame_el.checkbox(:name=>"preferences.showDateCreated") }
  element(:show_date_approved) { |b| b.frame_el.checkbox(:name=>"preferences.showDateApproved") }
  element(:show_current_route_nodes) { |b| b.frame_el.checkbox(:name=>"preferences.showCurrentNode") }
  element(:show_workgroup_request) { |b| b.frame_el.checkbox(:name=>"preferences.showWorkgroupRequest") }
  element(:show_doc_route_status) { |b| b.frame_el.checkbox(:name=>"preferences.showDocumentStatus") }
  element(:show_app_doc_status) { |b| b.frame_el.checkbox(:name=>"preferences.showAppDocStatus") }
  element(:show_clear_fyi) { |b| b.frame_el.checkbox(:name=>"preferences.showClearFyi") }
  element(:show_use_outbox) { |b| b.frame_el.checkbox(:name=>"preferences.useOutbox") }
  element(:save_el) { |b| b.frame_el.button(:name=>"methodToCall.save") }
  element(:reset_el) { |b| b.frame_el.link(:href=>"javascript:document.forms[0].reset()") }
  element(:cancel_el) { |b| b.frame_el.link(:href=>"ActionList.do") }
